    ...
    formacao: [
      {
        ...
        imagem: '/ilustração enconto 4.webp',
      },
      {
...
Ninguém te ama como Eu`,
  ...
  },
  {
    ...
    reflexao: [],
    compromisso: 'Me comprometo a participar de todos os encontros do Crisma de Adultos 2026 e a rezar durante esta semana: "Deus, eu sou seu filho e o Senhor é meu Pai."',
    recursos: [],
    quiz: [
      {
        pergunta: 'O que é o sacramento do Crisma?',
        opcoes: [
          'O primeiro sacramento recebido na vida cristã',
          'O sacramento que confirma e fortalece a fé recebida no Batismo',
          'Um rito de passagem sem vínculo com outros sacramentos',
          'O sacramento que celebra a reconciliação com Deus',
        ],
        correta: 1,
        explicacao: 'O Crisma (Confirmação) confirma e aprofunda a graça batismal, fortalecendo o fiel com os dons do Espírito Santo para viver e testemunhar a fé.',
      },
      {
        pergunta: 'Qual é o principal objetivo do Crisma de Adultos 2026?',
        opcoes: [
          'Estudar filosofia e teologia de forma acadêmica',
          'Percorrer um caminho de formação e fé rumo ao sacramento da Confirmação',
          'Participar de eventos sociais e comunitários',
          'Revisar os mandamentos e preceitos da Igreja',
        ],
        correta: 1,
        explicacao: 'O Crisma de Adultos propõe um caminho de encontros formativos e de aprofundamento da fé que culmina na celebração do sacramento da Confirmação.',
      },
    ],
  },
  {
...
// Retorna os dados do recesso se ele estiver ativo E ainda não tiver chegado a
// data de retorno; caso contrário, null (comportamento normal).
export function getRecessoAtivo(referencia = new Date()) {
  if (!RECESSO.ativo) return null
  const hoje = new Date(referencia)
  hoje.setHours(0, 0, 0, 0)
  const retorno = new Date(RECESSO.retorno + 'T00:00:00')
  if (hoje >= retorno) return null
  return { retorno }
}
